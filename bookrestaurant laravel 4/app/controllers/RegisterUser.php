		...
		if($_POST){
			...
			if($validation->fails()){
				...
				try{
					...
					$ruser->contact_telephone_number = Input::get('telephone_no');
				...
				DB::commit();
				Mail::send('emails.auth.activate', array('link' => URL::route('user-activate', $ruser->code),'name' => $ruser->first_name,'login' => $ruser->login_name), function($message) use ($ruser) {
