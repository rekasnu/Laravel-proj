		...
		<div class='t' data-ng-app='login'>
			...
			<div class='span4 offset4' data-ng-controller='logc'>
				...
				<div class="form-group">
					...
					<div class='error' data-ng-if='lo.login_name.$error.required && lo.login_name.$dirty'>
					...
					<div class='error' data-ng-if='lo.password.$error.required && lo.password.$dirty'>
						Password name is required.
					</div>
				</div>
				<div class='form-group'>
					{{ Form::submit('Sign in', array('data-ng-disabled'=>'lo.$invalid && !lo.dirty','class'=>'btn btn-success')) }}
					{{ HTML::link('user_register', 'Register', array('class'=>'btn btn-primary')) }}
					{{ HTML::link('reset', 'Forget password', array('class'=>'btn btn-warning')) }}
				</div>
				{{ Form::close() }}
				</div>	
			</div>
